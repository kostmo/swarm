...
--
-- The command-line interface for the @swarm-docs@ documentation generator,
-- factored out of the executable so it can be reused — for instance to render a
-- railroad diagram of the option grammar (see the @cli-args-render@ executable).
module Swarm.Doc.CLI (
  cliParser,
  cliInfo,
) where
...
import Swarm.Doc.Gen (EdgeFilter (..), GenerateDocs (..), PageAddress (..), SheetType (..))
